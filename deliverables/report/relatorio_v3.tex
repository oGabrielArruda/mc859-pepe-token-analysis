...
A descoberta central do projeto foi inesperada: as janelas temporais analisadas produziram \textbf{perturbações topológicas diametralmente opostas}, sugerindo que a natureza do choque --- e não apenas sua magnitude --- determina a assinatura estrutural da rede.
...
O algoritmo HITS~\cite{kleinberg1999}, originalmente proposto para grafos de citação na web, pode ser adaptado para redes de transação: um \textit{hub} é um endereço que envia para muitos destinos relevantes (distribuidor), e uma \textit{authority} é um endereço que recebe de muitos remetentes relevantes (acumulador). Utilizamos a implementação não-ponderada do NetworkX --- os scores refletem a \textit{estrutura de conectividade} (quem envia para quem), não o volume financeiro das arestas:
...
A convergência de múltiplos algoritmos sobre os mesmos endereços-chave --- Binance como destino dominante em ambos os eventos, BtcTurk como exchange regional com ascensão de ranking específica no Khamenei, e um endereço sinalizado como phishing na posição de hub mais influente durante volatilidade --- demonstra que a análise de grafos on-chain é uma ferramenta versátil para investigar tanto a dinâmica de mercado quanto potenciais riscos de segurança em criptoativos.
