...
\author{
  ...
  \small MC859: Projeto em Teoria da Computação, 2026S1 \\[6pt]
  \small Repositório: \url{https://github.com/oGabrielArruda/mc859-pepe-token-analysis}
}
\date{}
...
A SCC gigante cresceu de 21,7\% para 25,2\% e o $k_{\max}$ subiu de 8 para 10, indicando que o núcleo de agentes bidirecionais se tornou mais denso. O número de triângulos quase triplicou (1.935~$\to$~5.601) e as tríades do tipo 300 (ciclo bidirecional completo entre três agentes) saltaram de 78 para 494 ($+$534\%), consistente com atividade intensa de arbitragem ou trading cíclico entre agentes profissionais. A transitivity global, por outro lado, caiu ligeiramente (0,0021~$\to$~0,0018): na projeção não-dirigida sobre a qual ela é calculada, as arestas cresceram 88\% (20.478~$\to$~38.576), mas a expansão de grau dos hubs elevou ainda mais rápido o número de tripletos abertos (caminhos de comprimento 2), diluindo a fração de fechamento.
...
A Tabela~\ref{tab:agentes} sintetiza os endereços-chave identificados pela convergência de múltiplos algoritmos. A identificação via Etherscan foi realizada posteriormente, após os algoritmos apontarem independentemente para os mesmos endereços:
...
Este trabalho modelou a rede de transações do token PEPE como um grafo direcionado e ponderado e aplicou oito algoritmos, organizados em extração de conhecimento e detecção de padrões, sobre três instâncias: um \textit{baseline} de baixa volatilidade e duas janelas de choque (o sequestro de Maduro e o assassinato de Khamenei). Partimos de duas perguntas: se eventos de naturezas distintas produzem assinaturas topológicas diferenciáveis, e se movimentos abruptos são impulsionados por poucos vértices influentes ou por atividade distribuída. Os resultados permitem responder a ambas.

Quanto à primeira pergunta, a topologia muda de forma detectável e mensurável diante de eventos externos, e a direção da mudança varia com a natureza do evento. Na janela de incerteza prolongada (Maduro), o núcleo bidirecional se adensou: SCC, $k_{\max}$, triângulos e tríades 300 cresceram em conjunto; na janela contaminada por um evento de mercado (Khamenei, D$-$3), a periferia unidirecional se inflou, com aumento do componente IN, dos nós de grau~1 e do funil em direção a \textit{exchanges} centralizadas, enquanto a transitividade colapsou; o choque geopolítico isolado em mercado de baixa teve impacto estrutural limitado. As duas janelas, portanto, produziram perturbações opostas, sugerindo que a natureza do choque, e não apenas sua magnitude, está associada ao tipo de assinatura observada. A análise temporal foi decisiva nesse ponto: foi ela que permitiu decompor a janela Khamenei em um pulso de mercado de curta duração (dissipado em poucos dias) e o evento geopolítico subjacente, separação que uma fotografia estática do grafo agregado teria ocultado.

Quanto à segunda pergunta, a rede é fortemente concentrada em todos os regimes (Gini de volume $\approx 0{,}98$, com o top-20 chegando a 53\% do volume sob o evento Maduro) e tem cauda pesada consistente com regime \textit{scale-free} ($\alpha \approx 2{,}0$ no baseline). Os movimentos abruptos são conduzidos por poucos vértices de alta influência (hubs distribuidores e \textit{authorities} acumuladoras identificados pelo HITS), e não por atividade distribuída de forma homogênea. Graças a essa concentração, vários algoritmos apontaram, de forma independente, para os mesmos endereços. A Binance é o destino dominante nos dois eventos, mas em papéis distintos: \textit{sink} terminal no Maduro e \textit{authority} de alta conectividade no Khamenei. A BtcTurk aparece como \textit{exchange} regional, com salto de \textit{ranking} específico à janela iraniana. E um endereço marcado como \textit{phishing} ocupou a posição de hub mais influente justamente durante a maior alta de preço, um achado colateral que mostra que a análise de grafos também pode auxiliar a segurança on-chain, sinalizando padrões de distribuição anômalos para inspeção.

Quanto aos algoritmos aplicados, os de extração de conhecimento deram o panorama da rede e a leitura do fluxo. O ajuste de lei de potência mostrou o regime de cauda pesada, o PageRank ordenou os agentes pela influência financeira de seus fluxos, e a decomposição bow-tie separou o núcleo da periferia. A análise temporal, aplicando essas métricas dia a dia, foi o que permitiu enxergar o pulso de D$-$3 que o grafo agregado escondia. Os algoritmos de detecção de padrões, por sua vez, detalharam o que mudava dentro dessa estrutura: o k-core mediu a profundidade do núcleo, o censo de tríades identificou os circuitos fechados de arbitragem, a detecção de anomalia resumiu a ruptura do baseline em um escore comparável entre dias, e o HITS separou os papéis de hub e \textit{authority}, isolando os endereços-chave. A leitura de cada janela se sustenta porque algoritmos de tipos diferentes (estruturais, de fluxo, de concentração e temporais) levaram à mesma conclusão.

Por fim, o que permanece em aberto não é se a análise de grafos enxerga a estrutura de um choque, uma vez que isso foi demonstrado ao longo do trabalho, mas se a associação entre a natureza do choque e o tipo de assinatura, observada aqui em dois casos, se mantém como regularidade geral. Responder a isso exige mais eventos e diferentes janelas de observação, capazes de evitar a sobreposição de sinais que afetou a janela Khamenei.
