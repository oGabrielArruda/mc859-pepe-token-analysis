...
Os algoritmos de PageRank~\cite{page1999} e distribuição de grau~\cite{barabasi1999} foram utilizados via implementações da biblioteca NetworkX~\cite{networkx}, com parâmetros \texttt{weight="edge\_weight"} e \texttt{max\_iter=500} para o PageRank. O censo de tríades também segue a implementação do NetworkX. A análise temporal (Alg.~4) consiste na construção de sub-grafos diários a partir dos timestamps das arestas, com aplicação das métricas dos algoritmos 1--3 e 5--6 a cada sub-grafo. A seguir, apresentamos o pseudocódigo dos quatro algoritmos com adaptações específicas ao contexto de redes de transação.
...
O HITS revelou um hub dominante com score normalizado de 1,0: o endereço \texttt{0x301d9b...}, verificado via Etherscan como um contrato flagged \textbf{``Fake\_Phishing1685689''}. O mecanismo provável é o \textit{dust attack}: o contrato envia quantidades mínimas de tokens para milhares de carteiras de alto valor (authorities no grafo), aparecendo em seus históricos de transação para atrair interações fraudulentas. Isso explica por que o HITS o classifica como hub --- ele distribui para muitas authorities --- e não como authority. A descoberta de que o hub mais influente do período é um contrato malicioso demonstra que a análise de grafos pode detectar atividade predatória como subproduto da investigação de mercado.
...
O HITS no grafo completo Khamenei revelou uma inversão sem precedentes: Gini hub = 0,222 e Gini authority = 1,000. A authority máxima (score = 1,0) é o endereço \texttt{0x28c6c06298d514db089934071355e5743bf21d60}, verificado como a \textbf{hot wallet da Binance}. A inversão significa que milhares de hubs homogêneos (compradores de varejo) depositaram simultaneamente na mesma authority (Binance).

\subsubsection{BtcTurk e o nexo geopolítico}

O endereço \texttt{0xb334a6...}, terceiro no ranking de PageRank e no $k$-core máximo ($k=8$, percentil 100\%) do grafo Khamenei, foi verificado como a exchange turca \textbf{BtcTurk}. Esse endereço ocupa a posição 29 no baseline e sobe para a posição 3 no Khamenei --- uma variação de ranking que não se observa nos demais endereços de exchange. Uma hipótese para essa proeminência específica é a proximidade geopolítica da Turquia com o Irã, embora a verificação definitiva requeira dados de origem geográfica dos depositantes, não disponíveis on-chain.
...
O Maduro apresenta anomalias persistentes: todos os 7 dias têm pelo menos uma métrica com $|z| > 2$, e o anomaly score não retorna ao nível do baseline dentro da janela observada. No Khamenei, por outro lado, o anomaly score cai de 32,8 (D$-$3) para 7,5 em D$+$3, retornando à faixa de variação do baseline. Essa diferença de persistência reflete a natureza dos eventos: incerteza prolongada (Maduro) sustenta perturbação estrutural ao longo de dias, enquanto choque abrupto (Khamenei) é absorvido rapidamente pela rede.
...
A janela do Khamenei capturou inadvertidamente um evento de mercado (D$-$3), o que exigiu análise temporal para decompor os sinais --- essa sobreposição ilustra o desafio de isolar efeitos causais em dados observacionais. O baseline possui apenas 8 observações diárias, tornando os estimadores de média e desvio inerentemente instáveis; z-scores extremos como $z = 14{,}8$ devem ser interpretados qualitativamente (ruptura inequívoca do padrão normal), não como probabilidades precisas. Dados de preço intraday permitiriam correlação temporal mais fina. A identificação de endereços depende de bases externas (Etherscan), e o estudo mostra correlação topológica, não causalidade. Análises de sensibilidade --- como a verificação da estabilidade dos z-scores sob escolha de outro período de baseline --- são direções relevantes para trabalho futuro.
